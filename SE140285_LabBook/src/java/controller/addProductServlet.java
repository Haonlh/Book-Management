...
public class addProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (idproduct.length() < 2 || idproduct.length() > 10) {
                isError = true;
                error.setProductIDError("Product ID must be in 2-10");
            }
            if (!name.matches("^[a-zA-Z ]+$")) {
                ...
                isError = true;
                error.setProductNameError("Full name must be in 2-30");
            }
            ...
            if (quantity < 0 || quantity > 200) {
                isError = true;
                error.setQuantityError("Quantity should not be more than 200");
            }
            if (author.length() < 2 || author.length() > 20) {
                isError = true;
                error.setAuthorError("Author must be in 2-20");
            }
            if (description.length() < 2 || description.length() > 100) {
                isError = true;
                error.setDescriptionError("Author must be in 2-100");
            }
            if (isError) {
