    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!name.matches("^[a-zA-Z]+$")){
                check = true;
                error.setNameError(" Full name can not be a number !");
            ...
            if(address.length() < 5 || address.length()> 100){
                check = true;
                error.setAddressError(" Address must not out of 100 characters");
            }
            if(phone.matches("[a-zA-Z]+$")){
                check = true;
                error.setNameError("Phone Number can not be letter.");
            }else if(!phone.matches("^\\(?([0-9]{3})\\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$")){
                check = true;
                error.setPhoneError(" Phone number must be 10 charcters");
            }
            ...
            if(e.toString().contains("daplicate")){
